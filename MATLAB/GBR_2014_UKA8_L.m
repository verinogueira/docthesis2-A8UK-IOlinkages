...
% counterfactual: shock to low-skilled A8 stock
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% COUNTERFACTUAL: removing A8 immigrants to low-skilled labour supply 

%List of things changed in the counterfactual:
K_cf = K_obs;
L_cf = L_obs*0.968536647669954; % size of low-skilled stock in 2014 (APS) = -3.1463%
H_cf = H_obs;
...
% IO model
dqidL_cf = ((q_cf - q_obs)./q_obs)*100;

% no-IO model
dqidL_no = (deltai)*(-3.1463352330046);


% ratios non-IO over IO models' predictions
dqidL_ratios = dqidL_no./dqidL_cf;


% other calculated effects
dYdL_cf = ((Y_cf - Y_obs)/Y_obs)*100;
dwKdL_cf = ((wK_cf - wK_obs)/wK_obs)*100;
dwLdL_cf = ((wL_cf - wL_obs)/wL_obs)*100;
dwHdL_cf = ((wH_cf - wH_obs)/wH_obs)*100;
dkidL_cf = ((ki_cf - k_obs)./k_obs)*100;
dlidL_cf = ((li_cf - l_obs)./l_obs)*100;
dhidL_cf = ((hi_cf - h_obs)./h_obs)*100;
...
dkidL_model = 0;
dlidL_model = 0; 
dhidL_model = (-3.1463352330046); % size of the shock
dYdL_model = (delta)*(-3.1463352330046);
dwKdL_model = (delta)*(-3.1463352330046);
dwLdL_model = -(1-delta)*(-3.1463352330046);
dwHdL_model = (delta)*(-3.1463352330046);


% compare effects on qi
dqidL_model = (eye(N)-Gamma')\[(1-gammai).*(deltai)*(-3.1463352330046)];
if max(abs(dqidL_model - dqidL_cf)) >= 1e-1 
    disp('check dqidL_model')
end
