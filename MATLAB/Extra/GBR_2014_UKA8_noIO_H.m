%% MODEL WITHOUT IO
%counterfactual to high-skilled A8 below
...
%model's predictions
dYdH_model = (1-alpha-delta)*(-2.20800955164073)
dYdH_cf = ((Y_cf - Y_obs)/Y_obs)*100

dwKdH_model = (1-alpha-delta)*(-2.20800955164073)
dwKdH_cf = ((wK_cf - wK_obs)/wK_obs)*100

dwLdH_model = (1-alpha-delta)*(-2.20800955164073)
dwLdH_cf = ((wL_cf - wL_obs)/wL_obs)*100

dwHdH_model = (-alpha-delta)*(-2.20800955164073)
...
dhidH_model = (-2.20800955164073)
dhidH_cf = ((hi_cf - h_obs)./h_obs)*100


% calculate effects on qi
dqidH_model = (1-alphai-deltai)*(-2.20800955164073);
dqidH_cf = ((q_cf - q_obs)./q_obs)*100;
if max(abs(dqidH_model - dqidH_cf)) >= 1e-2 % error = 0.0061
    disp('check dqidH_model')
end


% PRICES: calculate effects on pi
dpi_dH_model = (-2.20800955164073).*((1-alpha-delta)-(1-alphai-deltai));
...
y = [h_obs';hi_cf'];
bar(industry,y')
title('High Skilled Labour'), xlabel('Industries'), ylabel('Thousands of Workers')
legend('Baseline','Counterfactual','Location','northwest')
%createfigure1(industry, y')
saveas(gcf,'H_values_UKA8H_noIO.png')


%% Export data
% CF = table(industry,alphai,deltai,gammai,gamma_out,y0,mu,beta,d_obs,d_cf,h_obs,k_obs,l_obs,hi_cf,ki_cf,li_cf,p_cf,p_data,q_obs,q_cf,y_obs,y_cf);
% filename = 'Counterfactuals.xls';
% writetable(CF,filename,'Sheet',1,'Range','B2')

filename = 'Counterfactuals_UKA8H_noIO.xls';

CF = table(industry,alphai,deltai,h_shares,mu,beta,ki_cf,li_cf,hi_cf,p_cf,q_cf,y_cf,dqidH_model,dqidH_cf);
...
% % % aggregate effect % % %
filename = 'Aggregate_Effect_UKA8_HnoIO.xls';
    HnoIO = table(industry,q_obs,q_cf);
        writetable(HnoIO,filename,'Sheet',1,'Range','B2')